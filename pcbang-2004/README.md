...
The ambience is built in the browser from oscillators, noise buffers written sample by sample
into an `AudioBuffer`, biquad filters, and scheduled envelopes. That is both a licensing
decision and the point of the piece.

A PC bang had no shared music — every seat had its own headset — so what you hear depends on
where you are sitting. `assets/js/room-music.js` puts the window seat's track through the
ceiling speakers by the door, the corner seat's through the vinyl-pad headset chained to that
desk, and the counter seat's through the owner's AM radio behind the counter. Moving seats
crossfades between them; there is no player and no next button.
...
recorded sound in the repository at all: the ambience is generated by the Web
Audio API at runtime from the code in `assets/js/audio.js`, and the three
tracks in `assets/audio/` were generated with MiniMax-Music3 on our own GPUs.
