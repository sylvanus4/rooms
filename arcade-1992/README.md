...
That split is a licensing requirement as much as an aesthetic one: none of this could ship
with real arcade music. The three tracks in `assets/audio/` are played through
`assets/js/room-music.js`, which puts two of them inside a plywood cabinet with one small
mono driver and the third through the ceiling from the rink upstairs, so they are objects in
the room rather than a soundtrack over it. Past the page's own files it makes no network
request of any kind.
...
MIT, see `LICENSE`. No copyrighted audio is used or distributed here. The ambience is
generated by `assets/js/audio.js` while you listen to it, and the three tracks in
`assets/audio/` were generated with MiniMax-Music3 on our own GPUs. Every track is
instrumental.
