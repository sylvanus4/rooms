...
const PROFILES = {
  ...
  /* ── A live band, in the same air as you, at four distances ──────────
   *
   * These four are not speakers, and that is the point of them. Nothing is
   * reproducing this music: seven men are playing it at the far end of a
   * narrow room with a pressed-tin ceiling. So every one of them has
   * `hiss: 0`. A medium's hiss belongs to the device, and there is no
   * device — the noise floor of this room is the crowd, and the crowd is
   * synthesised by the room engine rather than carried by the music.
   *
   * What actually changes as you move away from a band indoors is not air.
   * Over twelve metres, molecular absorption at 4 kHz costs on the order of
   * a decibel, which nobody notices. Three other things cost a great deal,
   * and these profiles are those three rather than a distance knob:
   *
   *   1. Direct-to-reverberant ratio. Past the critical distance of a small
   *      hard room you are mostly listening to the walls, so `send` climbs
   *      much faster than `lp` falls. Distance is mostly a reverb parameter.
   *   2. Horn directivity. A cornet throws its 2-4 kHz down the bell axis
   *      and very little of it sideways, so stepping to the side of the
   *      stage takes the top off before any distance has a chance to.
   *   3. The crowd. Bodies, wool coats and hats are broadband absorbers,
   *      and a packed room between you and the front line eats the presence
   *      region first and hardest.
   *
   * Only the two far profiles carry a compressor, and it is not a device
   * either: a reverberant far field genuinely evens out dynamics, because
   * the tail of the last note fills the gap before the next one.
   */

  // Standing at the stage lip. Nothing between you and the bells. The tuba
  // and the bass drum arrive as pressure rather than as pitch, and the
  // cornet's on-axis 2.4 kHz is close to painful, which is why nobody who
  // wanted to talk ever sat here.
  'bandstand-front': {
    hp: 28, lp: 16000, lpQ: 0.6,
    peaks: [{ f: 2400, gain: 4, q: 1.1 }, { f: 95, gain: 2.5, q: 0.9 }],
    send: 0.06, hiss: 0,
  },

  // Seven metres out and well off the bell axis, at the bar. The horns are
  // pointing past you, so the top goes first; what is left is the body of
  // the instruments plus boundary gain off a hard bar back and its mirror,
  // which reinforces the low mids more than a free field would.
  'bar-side': {
    hp: 42, lp: 7200, lpQ: 0.7,
    peaks: [{ f: 3200, gain: -5, q: 0.9 }, { f: 800, gain: 3, q: 0.9 }, { f: 165, gain: 2.5, q: 1.2 }],
    send: 0.2, hiss: 0,
  },

  // Twelve metres back with the whole room in between. The low end is the
  // part that survives — a narrow room's length modes pile up in the low
  // mids — and the presence region is the part the crowd absorbs, so the
  // rolloff is steeper than air alone would give.
  'back-room': {
    hp: 38, lp: 4600, lpQ: 0.7, lpStages: 2,
    peaks: [{ f: 2600, gain: -4, q: 0.8 }, { f: 200, gain: 3.5, q: 1.0 }],
    comp: { threshold: -26, ratio: 2, attack: 0.02, release: 0.4 },
    send: 0.52, hiss: 0,
  },

  // Sitting on the stairs up to the street. The band is around a corner and
  // below you, so everything arrives through one doorway: mono, because a
  // single aperture cannot carry a stereo image, and peaked hard where the
  // stair shaft's own axial mode is. Almost all of what you hear is tail.
  'stairwell': {
    hp: 60, lp: 1400, lpQ: 1.4, lpStages: 2, mono: true,
    peaks: [{ f: 300, gain: 6, q: 1.6 }, { f: 900, gain: -4, q: 1.0 }],
    comp: { threshold: -24, ratio: 3, attack: 0.015, release: 0.45 },
    send: 0.78, hiss: 0,
  },
};
...
export const SLOT_MEDIUM = {
  ...
  // 1927, a hot-jazz club. The band is in the room with you, so the key is
  // not which device plays the track but where you are standing relative to
  // seven men playing it. One track, four profiles, and the room engine
  // builds all four at once and crossfades between them when you move.
  'neworleans-1927__set-1__front':  'bandstand-front',
  'neworleans-1927__set-1__bar':    'bar-side',
  'neworleans-1927__set-1__back':   'back-room',
  'neworleans-1927__set-1__stairs': 'stairwell',
};
